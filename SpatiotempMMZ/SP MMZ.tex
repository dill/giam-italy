...
%\textbf{Interpreting the usefulness of the proposed tool in a metaphorical way, it is as if the scholar has the opportunity to read the empirical maps in Figure \ref{Rd} wearing ``special spectacles'', that allows him/her to see in a more clear and sharp manner the spatial and temporal patterns of the IRF. DON'T LIKE THIS! DON'T THINK WE NEED IT.}
...
%Comparison between the soap film model and a version of model 1 using a combination of thin plate splines and curbic regression as well as a tensor product of cubic regression splines did not effectively highlight the differences shown in the maps in figure {}.  Although these extra models showed the same general trends as the soap film, the soap film 
...
The search for employment is well known to be among the main reasons that individuals move to another country (e.g., Fullin and Reyneri 2011). In light of this, labor market and income indicators could help explain the spatial heterogeneity in the IRF observed in Figure \ref{fig1}. Unfortunately, the additional variables to be included into model (\ref{PropM}) are available at a different spatial resolution: such economic indicators (unemployment and employment rates, and gross domestic product (GDP) per capita) are available at a provincial level (around 100 observations per year), while the IRF is available at a municipal level (approximately 8000 observations per year). 

As a descriptive analysis, we constructed a simple indicator of spatial economic attractiveness (ISEA) based on the unemployment and employment rates, and real GDP per capita using provincial data. Further details are given in the Appendix. We then produced a boxplot of this indicator for each typical Italian macro-area (see Figure \ref{fig3}). We show only the boxplots for 2008 because there were not significant differences across the different years. 
