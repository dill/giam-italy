...
\title{Modelling the Spatiotemporal Distribution of the Incidence of Resident Foreign Population Using a Soap Film Smoother}
...
Recently, many European countries have experienced a substantial increase in the proportion of immigrants in their populations. The incidence of resident foreigners calculated at a national level does not provide information on the spatial and temporal distribution of the phenomenon, which may be crucial to demographers, economists, sociologists, and policy makers to study and formulate adequate policies supporting, e.g., the process of integration of resident foreigners in a host country. In this paper we suggest a tool for practitioners to provide spatiotemporal maps representing, at a local level, the distribution of the incidence of resident foreigners in the territory, and changes in spatial trends over time. In doing this, we use Italian data at municipal level, for the period 2003-2008. The temporal trend of the incidence occurs at different strengths and patterns depending on the spatial location hence it is necessary to allow for such interaction in the model. We propose using a generalized additive model incorporating a scale-invariant smoother of the time and space dimensions. Specifically, we set up a tensor product smoother combining a cubic regression spline basis for time and a soap film spline basis for space. The employed approach provides a consistent framework to produce spatiotemporal maps which could be effectively used by policy makers to decide the allocation of economic resources at local level.

\vspace{0.4cm}
\noindent \textbf{Key Words:} Generalized additive model; Incidence of resident foreigners; Policy-making; Soap film spline basis; Spatiotemporal maps; Tensor product smoother. 
...
Here we consider the Italian case. According to the Italian National Statistical Office (ISTAT), the IRF has grown substantially, from 2.7\% in 2002 to 6.5\% in 2008. This phenomenon has had an impact on the population, especially from a socio-demographic point of view (see Section \ref{DEM} for details). The IRF calculated on a national level does not tell us anything specific about the local spatial distribution of the phenomenon. Demographers, economists, sociologists, and policy makers should find local information on how the distribution of the IRF changes over time and space useful during the process of studying and formulating adequate policies supporting, e.g., the process of integration of resident foreigners in a host country. 

\begin{figure}[tbp]
	\centering
		\includegraphics[width=0.85\textwidth, angle=270]{Raw.ps}
	\caption{Empirical maps of the percentage incidence of resident foreigners in Italy over the years 2003--2008. These were obtained using data at a municipal level.	\textbf{DAVE Define colour scale}.}
	\label{Rd}
\end{figure}

To the best of our knowledge, no studies have attempted to model the spatiotemporal distribution of the IRF in a country. Our study uses annual Italian data for the period 2003-2008. Since 2003, ISTAT has provided a new public database with annual frequency of the number of resident foreigners at a municipal level. This represents the current maximum spatial and temporal detail available. \textbf{Figure \ref{Rd} shows the empirical maps of the IRF at a municipal level over the years 2003--2008. The features that stand out clearly are the marked difference between north and south, and the increase in the IRF over time. However, these maps make it really hard to detect the existence of important local spatial patterns. For example, in 2008, in the Po Valley, which is located in northern Italy, we observe that the whole area is characterized by medium-high IRF levels but it is not really possible to identify the existence of polarization patterns. This is problematic because, at a local level, policy makers would most probably allocate economic resources in an indistinct way given that, based on the empirical maps, it does not seem that there exist particular differences in this geographical area.} Our emphasis is to suggest a tool for practitioners to provide clearer spatiotemporal maps representing the distribution of the IRF in the territory and changes in spatial trends over time, at a local level. 

Methods for spatiotemporal modelling involves mainly hierarchical Bayes models, geostatistical models and generalized additive models (e.g., Augustin et al. 2009; Banerjee, Carlin, and Gelfand 2004; Hastie and Tibshirani 1990; Kamman and Wand 2003; Kneib and Fahrmeir 2006; Osei, Duker, and Stein 2011; Ruppert, Wand, and Carroll 2003; Sahu, Gelfand, and Holland 2007; Wood 2006). Here we opt for generalized additive models since they allow a wide range of possibilities for representing the model component functions, which is crucial in our case study to fulfill the model requirements, as briefly detailed below. Because currently no relevant economic data are available at municipal level, the response (IRF) is only modelled using a smoother which is function of the spatial coordinates and time. The model is then used to create smoothed maps of the geographical area of interest over time. Two points are noteworthy here. First, when a geographical region has complex boundaries, such as Italy for example, a phenomenon known as \textit{leakage} typically occurs (Ramsay 2002). In this case, a smoother would inappropriately link two parts of a domain causing the fitted surface to be mis-estimated, leading to incorrect inference (e.g., biased incidence estimates), which is clearly not desirable. This issue can be overcome if the smoother used can account for the structure of the domain that is under investigation; several solutions have been proposed (e.g., Ramsay 2002; Wood, Bravington and Hedley 2008, and references therein). Second, because it is unlikely that the IRF occurs at similar strengths and patterns over the territory, the employed approach should account for a space-time interaction. These two goals can be achieved by using a generalized additive model incorporating a scale-invariant three-dimensional tensor product smoother combining a cubic regression spline basis function for the temporal trend and a soap film spline basis for the spatial component (Wood 2006; Wood, Bravington and Hedley 2008).
...
$$
where the $\textbf{S}$ matrices contain known coefficients whose values depend on the chosen bases for time and space. For instance, the second-order cubic spline penalty for $f_\text{year}(\texttt{year})$ evaluates $J_\text{year}(f_\text{year})=\int\left( \partial^2 f_\text{year}/\partial \texttt{year}^2 \right)^2 \text{d}\texttt{year}$, but it may be more complex (Wood 2006). Following, e.g., Augustin et al. (2009), an overall penalty for the tensor product smoother can be obtained by applying the penalties of $f_\text{space}(\texttt{n},\texttt{e})$ to the varying coefficients of the marginal smooth $f_\text{year}(\texttt{year})$, $\xi_l(\texttt{n},\texttt{e})$,
$$
...
For the internal part of the smoother we first find a set of functions $\rho_k(\texttt{n},\texttt{e})$, which are each solutions to the Laplace's equation in two dimensions
$$
...
As with the basis, the penalty is split into two parts: one for the cyclic smoother around the boundary and one for the internal smoother. Writing $\bm\beta$ as the vector of all smooth coefficients for the soap film, $\bm\gamma$ for the boundary smooth parameters and $\bm\alpha$ for the interior, we have
$$
...
\label{Dev}
\eeq
where $\bm\theta$ contains all tensor product smooth parameters associated with the one-dimensional spline basis for time and two-dimensional smoother for space, and $\textbf{S}^*=\sum_i \lambda_i \textbf{S}_i$, where the $\textbf{S}_i$ are matrices of known coefficients properly defined according to the results of the previous sections with associated smoothing parameters $\lambda_i$. The model deviance, $D$, is defined as $2\phi(l_{\text{sat}}-l)$, where $\phi$ is a dispersion parameter, $l$ is the log-likelihood of the model and $l_{\text{sat}}$ the maximum value for the log-likelihood of the model with one parameter per datum. 
...
As mentioned in the introductory section, the IRF in Italy was 6.5\% in 2008. Obviously, this number tells us nothing about specific local areas with particularly high or low levels of resident foreigners. As we move to smaller aggregations (e.g., regional or provincial level) the same argument holds: some information is lost. This kind of low resolution statistic can mask smaller-scale heterogeneity in the population. Our aim here is to capture exactly this spatial heterogeneity using the approach described in the previous sections, and annual data on the number of resident foreigners at a municipal level.

\begin{figure}[tbp]
	\centering
		\includegraphics[width=0.85\textwidth, angle=270]{maps-Soap.ps}
	\caption{Spatiotemporal maps of the percentage incidence of resident foreigners in Italy over the years 2003--2008. These were obtained using model (\ref{PropM}) with a tensor product smoother based on a cubic regression spline basis for time and a soap film spline basis for space, and data at a municipal level. Predictions were made over those points lying inside the study region from a 100 by 100 grid.}
	\label{fig1}
\end{figure}

\textbf{Figure \ref{fig1} shows smoothed spatiotemporal maps of the IRF in Italy for the period 2003-2008, obtained using the approach described in the previous sections. As in Figure \ref{Rd}, we can see that the distribution of the IRF is not homogeneous over space and varies over time, and that there is a striking difference in incidence between north and south. However, as pointed out in the introduction, smoothed maps can describe with a much greater accuracy the distribution of the IRF in the territory and changes in spatial trends over time, at a local level.} From Figure \ref{fig1}, we can see that in 2003 northern and central Italy show the highest IRF. Over the subsequent years (2004-2008), the incidence spreads, forming four main areas where resident foreigners tend to live. Focusing on 2008, the most popular areas are in north Italy, specifically the Emilia-Romagna and Lombardy regions. These, although popular in 2003, appear to have greatly expanded. The second most attractive area is made up of the central Italian regions of Tuscany, Umbria, Marche and Lazio. The final two areas are composed of Liguria and Piedmont, and Veneto, Friuli-Venezia Giulia and Trentino-Alto Adige. These are located in the northwest and northeast of the country, respectively. There is also an interesting growth in the incidence at the Swiss and Austrian borders (from about 2\% in 2003 to about 4-5\% in 2008). Resident foreigners seem less attracted by the regions and islands of southern Italy. \textbf{Referring to the case of the Po Valley discussed in the introduction, the maps in Figure \ref{fig1} allow us to identify the presence of two main well defined polarization patterns of the IRF, the first in the centre of the Po Valley and the second near Venice. The maps in Figure \ref{Rd} can not reveal the patterns described above.} 

The spatiotemporal patterns shown in Figure \ref{fig1} can be useful for policy-makers to decide the allocation of resources at local level, supporting policies and services needed for the process of integration of resident foreigners. In this regard, we would expect an area with a high density of resident foreigners to require more resources than an area with a lower density. Such policies relate to a number of services such as: admission to education, access to the public health service, professional training, services supporting the match of labor supply and demand (e.g., Zincone 2006). For sociologists and demographers these maps may represent a new way to model spatiotemporal demographic changes and display the results graphically. \textbf{Interpreting the usefulness of the proposed tool in a metaphorical way, it is as if the scholar has the opportunity to read the empirical maps in Figure \ref{Rd} wearing ``special spectacles'', that allows him/her to see in a more clear and sharp manner the spatial and temporal patterns of the IRF.}
...
Figure \ref{trends}, shows the estimated temporal trends for both the full Italian territory and broken down by area with 95\% confidence intervals. The trends for northern and central Italy are similar. However, those for southern Italy and the islands are much flatter. Northern and central Italy also show a faster growth in incidence, this difference is supported by the confidence intervals. Overall, these trends reflect what we see in the maps in Figure \ref{fig1}, that there is a significant difference between northern/central Italy and the south of the country and its islands. Note that, since the approach outlined in Section \ref{VE} can not take into account the presence of residual correlation structure, we expect the obtained confidence intervals not to be exactly representative than those that might be produced using some adjusted version of them. As in Augustin et al. (2009), the most obvious approach would be to fit a GAMM with a carefully chosen correlation structure and use the resulting posterior covariance matrix in place of that reported in Section \ref{VE}. Unfortunately, as pointed out in the previous section, convergence problems prevented us from doing this. However, we doubt the conclusions would change.

\textbf{Figure \ref{fig1TPRS} shows the spatiotemporal maps obtained when using model (\ref{PropM}) with a tensor product smoother based on a cubic regression spline basis for time and a thin plate regression spline (TPRS) basis for space (e.g., Augustin 2009; Ruppert, Wand, and Carroll 2003; Wood 2006). Basis sizes for the TPRS and cubic regression spline bases were the same as those reported in the last two columns of Table \ref{soap-basis-table}. The main issue with this simpler approach is that a TPRS basis for the spatial dimension can not take into account the physical and administrative boundaries of the geographical area of interest; this can have a detrimental impact on the resulting estimates since such an approach would suffer from the problem of leakage (e.g., Wood, Bravington and Hedley 2008).}

\begin{figure}[tbp]
	\centering
		\includegraphics[width=0.85\textwidth, angle=270]{maps-TPRS.ps}
	\caption{Spatiotemporal maps of the percentage incidence of resident foreigners in Italy over the years 2003--2008. These were obtained using model (\ref{PropM}) with a tensor product smoother based on a cubic regression spline basis for time and a thin plate regression spline basis for space, and data at a municipal level. Predictions were made over those points lying inside the study region from a 100 by 100 grid.}
	\label{fig1TPRS}
\end{figure}

\textbf{Comparing the maps in Figure \ref{fig1} with those in Figure \ref{fig1TPRS}, we see that the use of the approach based on a TPRS basis masks several important features highlighted in the maps obtained using a soap film spline basis. This is problematic; the maps in Figure \ref{fig1TPRS} can lead to an incorrect interpretation of the demographic phenomenon over space as well as time, hence biasing the decision of the allocation of economic resources at local level. As an example, referring again to the case of the Po valley, the maps in Figure \ref{fig1} show two main well defined polarization patterns of the IRF and a detailed representation over space of its different levels. These considerations can not be drawn from the maps in Figure \ref{fig1TPRS}. This suggests that, for this case study, the use of a simple approach does not represent a valid solution to capture important features of the local spatial distribution of the IRF. We also fitted model (\ref{PropM}) incorporating a tensor product smoother based on three cubic regression spline bases for time, longitude and latitude, but the conclusions did not change.}


\subsection{Spatial economic attractiveness \label{diss}}

In the literature it is well known that among the main reasons that encourage individuals to move to another country is the search of employment for improving, e.g., the personal and/or family members well-being status (e.g., Fullin and Reyneri 2011). In light of this consideration, the main indicators of the labor market and of income could help to explain the spatial heterogeneity of the IRF observed in Figure \ref{fig1}. Unfortunately, economic indicators such as unemployment and employment rates, and gross domestic product (GDP) per capita are not available at a municipal level. This is an important limitation of the official statistics. For this reason, we can not incorporate this information into model (\ref{PropM}) given the differing spatial resolution of the quantities being modelled; the IRF is available at a municipality level (this means that for the IRF there are about 8000 observations available per year), while the economic indicators at a provincial level (about 100 observations per year).

As a descriptive analysis, we constructed a simple indicator of spatial economic attractiveness (ISEA) based on the unemployment and employment rates, and real GDP per capita using provincial data (the maximum spatial resolution available for these indicators). Further details are given in the Appendix. We then produced a boxplot of this indicator for each typical Italian macro-area (see Figure \ref{fig3}). We show only the boxplots for macro-area relative to 2008 because there were not significant differences across the different years. 
