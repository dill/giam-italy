...
Figure \ref{fig1TPRS} shows the spatiotemporal maps obtained with model (\ref{PropM}) but with a tensor product smooth of cubic regression spline basis for time and a thin plate regression spline (TPRS) basis for space (e.g., Augustin 2009; Ruppert, Wand, and Carroll 2003; Wood 2006). Basis sizes for the TPRS and cubic regression spline bases were the same as those reported in the last two columns of Table \ref{soap-basis-table}. The main issue with this simpler approach is that a TPRS basis for the spatial dimension can not take into account Italy's physical and national boundaries; as mentioned above, this can have a detrimental impact on the resulting estimates (via  leakage (e.g., Ramsay 2002,  Wood, Bravington and Hedley 2008)).
...
Comparing the maps in Figure \ref{fig1} with those in Figure \ref{fig1TPRS}, we see that the TPRS basis approach masks several important features highlighted in the maps obtained using a soap film spline basis. This is problematic; the maps in Figure \ref{fig1TPRS} can lead to an incorrect interpretation of the demographic phenomenon over space as well as time, hence biasing the decision of the allocation of economic resources at local level. \textbf{Using the Po Valley again as an example, the maps in Figure \ref{fig1} show two well defined polarization patterns of the IRF and a detailed representation over space of its different levels. These considerations can not be drawn from the maps in Figure \ref{fig1TPRS}. This suggests that, for this case study, the use of a simple approach does not represent a valid solution to capture important features of the local spatial distribution of the IRF. We also fitted model (\ref{PropM}) incorporating a tensor product smoother based on three cubic regression spline bases for time, longitude and latitude, but the conclusions did not change.HRM!}
