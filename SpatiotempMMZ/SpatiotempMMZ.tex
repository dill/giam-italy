...
The soap film smoother (CITE: Wood et al) uses a rather simple physical model to prevent leakage from occurring. First, consider the domain boundary to be made of wire, then dip this wire into a bucket of soapy water, you will then have (provided it doesn't pop(!)) a soap film in the same of your boundary. Now consider the wire to lie in the $\texttt{n}-\texttt{e}$ plane and the height of the soap film at a given point to be the functional value of the model. This film is then distorted smoothly by moving it toward the data, while minimising the surface tension in the film. The domain ($\Omega$) is bounded by some polygon ($B$) which, in this case is the country borders. The boundary conditions that are estimated by a cyclic spline (CITE Simon book, p 151).
...
\label{soap-poisson}
\eeq
with $\rho=\rho_k(\texttt{n},\texttt{e})$, where $k$ indexes the knots and the boundary condition $\rho=0$. The set of basis functions for the soap film smoother, $g_k(\texttt{n},\texttt{e})$ is found, along with $a(\texttt{n},\texttt{e})$ (the solutions to (\ref{soap-poisson}) when $\rho=0$, subject to the boundary condition). These bases are then summed to form:
\beq
f_\text{int}(\texttt{n},\texttt{e})=a(\texttt{n},\texttt{e})+\sum_{k=1}^n \gamma_k g_k(\texttt{n},\texttt{e}),
\eeq
the soap film smoother for the interior, where the $\gamma_k$ are parameters to be estimated. 

The boundary smooth is calculated as in Wood (book p151) with the form
\beq
...
\eeq
where $S_\text{space}$ is the total penalty, $S_\text{int}$ is the interior penalty and $S_\text{bnd}$ is the cyclic spline boundary penalty. The $\lambda$ are the smoothing parameters for the boundary and interior smooths respectively. We now look at the two parts of the penalty individually.
